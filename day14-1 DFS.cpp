...
public:
	...
	void insert_vertex(int n,char cc) {						//고유번호가 n인 vertex를 그래프에 삽입
		if (vertexSize != 0) {
			if (isFindVertex(n) == true)return;
			else {
				vertex* newVertex = new vertex(n,cc);
...
void DFS(vertex* V) {
	edge* it = V->incidentEdgeList->head;
	while (1) {
		if (it->back == false && it->discovery == false)
		{
			vertex* w = it->destination == V ? it->source : it->destination;
			if (w->visited == false) {
				it->discovery = true;
				w->visited = true;//
				cout << w->c;//
...
int main() {
	...
	for (int i = 0; i < N; i++) {
		cin >> I >> C;
		myG->insert_vertex(I,C);

	}

	for (int i = 0; i < M; i++) {
		cin >> S >> D;
		/*if (myG->isFindEdge(S,D) == true)cout << -1 << endl;
		else*/ myG->insert_edge(S, D);
	}
	vertex* it = myG->TotalvertexList->head;
	while (1) {
		if (it->visited == false)
			DFS(it);
		if (it != myG->TotalvertexList->tail)
			it = it->next;
		else break;
	}
	//cout << myG->TotalvertexList->size << " " << g->TotaledgeList->size << endl;

	/*for (int i = 0; i < K; i++) {
		cin >> temp1 >> temp2;
		if (g->isFindVertex(temp1) == false || g->isFindVertex(temp2) == false)cout << -1 << endl;
		else {
